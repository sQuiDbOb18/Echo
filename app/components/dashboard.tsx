...
function positionLabel(shares: bigint | undefined, totalShares: bigint | undefined, nav: bigint | undefined) {
  if (shares === undefined || totalShares === undefined || nav === undefined || totalShares === 0n) return "--";
  return `$${Number(formatUnits((nav * shares) / totalShares, 6)).toLocaleString(undefined, { maximumFractionDigits: 2 })}`;
}

function AgentCard({ agent, nav, trades, shares, totalShares, isConnected, mounted }: { agent: AgentKey; nav?: bigint; trades: Trade[]; shares?: bigint; totalShares?: bigint; isConnected: boolean; mounted: boolean }) {
  ...
  return <article className="relative overflow-hidden rounded-[2px] border border-white/10 bg-[#11161d] p-6 shadow-2xl shadow-black/20">
    ...
    <div className="relative mt-8 grid grid-cols-2 items-end gap-4"><div><p className="font-sans text-[10px] uppercase tracking-[0.2em] text-white/35">Current NAV</p><p className="mt-1 text-2xl">{navLabel(nav)}</p>{mounted && isConnected && <div className="mt-4 font-sans text-xs text-white/45"><p className="uppercase tracking-[0.16em] text-white/35">Your position</p><p className="mt-1 text-white/75">{shares === undefined ? "--" : `${formatUnits(shares, 6)} shares · ${positionLabel(shares, totalShares, nav)}`}</p></div>}</div><div className="h-16"><ResponsiveContainer width="100%" height="100%"><AreaChart data={points}><defs><linearGradient id={`${agent}Fill`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={agents[agent].accent} stopOpacity={0.35} /><stop offset="100%" stopColor={agents[agent].accent} stopOpacity={0} /></linearGradient></defs><Area type="monotone" dataKey="value" stroke={agents[agent].accent} fill={`url(#${agent}Fill)`} strokeWidth={2} /></AreaChart></ResponsiveContainer></div></div>
  </article>;
}
...
export function Dashboard() {
  ...
  const momentumShares = useReadContract({ address: contracts.momentum, abi: vaultAbi, functionName: "shares", args: address ? [address] : undefined, chainId: baseSepolia.id, query: { ...liveReadOptions, enabled: Boolean(contracts.momentum && address) } });
  const contrarianShares = useReadContract({ address: contracts.contrarian, abi: vaultAbi, functionName: "shares", args: address ? [address] : undefined, chainId: baseSepolia.id, query: { ...liveReadOptions, enabled: Boolean(contracts.contrarian && address) } });
  const momentumTotalShares = useReadContract({ address: contracts.momentum, abi: vaultAbi, functionName: "totalShares", chainId: baseSepolia.id, query: { ...liveReadOptions, enabled: Boolean(contracts.momentum) } });
  const contrarianTotalShares = useReadContract({ address: contracts.contrarian, abi: vaultAbi, functionName: "totalShares", chainId: baseSepolia.id, query: { ...liveReadOptions, enabled: Boolean(contracts.contrarian) } });
  ...
  async function refreshAfterTransaction() {
    await Promise.all([
      momentumNav.refetch(),
      contrarianNav.refetch(),
      momentumShares.refetch(),
      contrarianShares.refetch(),
      momentumTotalShares.refetch(),
      contrarianTotalShares.refetch(),
    ]);
  ...
  return <main className="min-h-screen bg-[#080b10] selection:bg-[#d7ff65] selection:text-black">
    ...
    <div className="mx-auto max-w-7xl px-6 pb-16 pt-12 lg:px-10 lg:pt-20"><header className="max-w-3xl"><p className="font-sans text-xs uppercase tracking-[0.28em] text-[#d7ff65]">Signal, made visible</p><h1 className="mt-5 text-5xl leading-[0.95] tracking-tight sm:text-7xl">Let the strategy<br /><em className="text-white/45">speak for itself.</em></h1><p className="mt-7 max-w-xl font-sans text-sm leading-7 text-white/50">Two autonomous paper portfolios. Every rotation is public, priced by mock liquidity, and explained in plain language.</p></header>
      ...
      <section className="mt-14 grid gap-5 md:grid-cols-2"><div><AgentCard agent="momentum" nav={momentumNav.data} trades={trades.filter((trade) => trade.agent === "Momentum")} shares={momentumShares.data} totalShares={momentumTotalShares.data} isConnected={Boolean(address)} mounted={mounted} />{readStatus(momentumNav.error) && <p className="mt-2 font-sans text-xs text-white/40">{readStatus(momentumNav.error)}</p>}</div><div><AgentCard agent="contrarian" nav={contrarianNav.data} trades={trades.filter((trade) => trade.agent === "Contrarian")} shares={contrarianShares.data} totalShares={contrarianTotalShares.data} isConnected={Boolean(address)} mounted={mounted} />{readStatus(contrarianNav.error) && <p className="mt-2 font-sans text-xs text-white/40">{readStatus(contrarianNav.error)}</p>}</div></section>
