...
function AgentCard({ agent, nav, trades, shares, totalShares, isConnected }: { agent: AgentKey; nav?: bigint; trades: Trade[]; shares?: bigint; totalShares?: bigint; isConnected: boolean }) {
  ...
  return <article className="relative overflow-hidden rounded-[2px] border border-white/10 bg-[#11161d] p-6 shadow-2xl shadow-black/20">
    ...
    <div className="relative mt-8 grid grid-cols-2 items-end gap-4"><div><p className="font-sans text-[10px] uppercase tracking-[0.2em] text-white/35">Current NAV</p><p className="mt-1 text-2xl">{navLabel(nav)}</p>{isConnected && <div className="mt-4 font-sans text-xs text-white/45"><p className="uppercase tracking-[0.16em] text-white/35">Your position</p><p className="mt-1 text-white/75">{shares === undefined ? "--" : `${formatUnits(shares, 6)} shares · ${positionLabel(shares, totalShares, nav)}`}</p></div>}</div><div className="h-16"><ResponsiveContainer width="100%" height="100%"><AreaChart data={points}><defs><linearGradient id={`${agent}Fill`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={agents[agent].accent} stopOpacity={0.35} /><stop offset="100%" stopColor={agents[agent].accent} stopOpacity={0} /></linearGradient></defs><Area type="monotone" dataKey="value" stroke={agents[agent].accent} fill={`url(#${agent}Fill)`} strokeWidth={2} /></AreaChart></ResponsiveContainer></div></div>
  </article>;
}
